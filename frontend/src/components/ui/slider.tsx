    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="block w-8 h-2 bg-amber-400 border border-black rounded-sm shadow-md hover:brightness-110 focus:outline-none focus:ring-0.5 focus:ring-black transition"
        />
      ))}
    </SliderPrimitive.Root>
  );
